...
# Importamos la función base para un control más estable
...
# 🛰️ CAPTURA DE GPS GLOBAL (Solución definitiva al bloqueo)
# Se ejecuta una sola vez al cargar la página para evitar bucles.
...
def registrar(nombre, tipo):
    ...
    try:
        ...
        st.error("❌ Error al leer sensor GPS."); return
    ...
    # 3. 🗺️ Geocerca (Acceso corregido a la tabla sucursales)
    ...
    if res_suc.data:
        ...
        if dist > radio_p:
            ...
            st.success(f"📍 Ubicación válida: {dist:.0f}m de la sucursal.")
